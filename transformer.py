from torch import nn
from torch import Tensor
from torch.nn import Transformer
import torch

import numpy as np


class PositionalEncoding(nn.Module):
    def __init__(self,
                 emb_size,
                 maxlen = 5000):
        super(PositionalEncoding, self).__init__()
        den = torch.exp(- torch.arange(0, emb_size, 2)* np.log(10000) / emb_size)
        pos = torch.arange(0, maxlen).reshape(maxlen, 1)
        pos_embedding = torch.zeros((maxlen, emb_size))
        pos_embedding[:, 0::2] = torch.sin(pos * den)
        pos_embedding[:, 1::2] = torch.cos(pos * den)
        self.pos_embedding = pos_embedding

    def forward(self, token_embedding: Tensor):
        return token_embedding.to(device) + self.pos_embedding.unsqueeze(1)[:token_embedding.shape[0], :].to(device)


class CustomTransformer(nn.Module):
    def __init__(self, num_encoder_layers, num_decoder_layers, emb_size, nhead,
                 src_vocab_size, tgt_vocab_size, dim_feedforward, dropout=0.1):
        super(CustomTransformer, self).__init__()
        self.transformer = Transformer(d_model=emb_size,
                                       nhead=nhead,
                                       num_encoder_layers=num_encoder_layers,
                                       num_decoder_layers=num_decoder_layers,
                                       dim_feedforward=dim_feedforward,
                                       dropout=dropout)
        self.linear = nn.Linear(emb_size, tgt_vocab_size)
        self.src_emb = nn.Embedding(src_vocab_size, emb_size)
        self.tgt_emb = nn.Embedding(tgt_vocab_size, emb_size)
        self.positional_encoding = PositionalEncoding(emb_size)

    def forward(self, src, trg, src_mask, tgt_mask,
                src_padding_mask, tgt_padding_mask,
                memory_key_padding_mask):
        src_emb = self.positional_encoding(self.src_emb(src))
        tgt_emb = self.positional_encoding(self.tgt_emb(trg))
        out = self.transformer(src_emb, tgt_emb, src_mask, tgt_mask, None,
                                src_padding_mask, tgt_padding_mask, memory_key_padding_mask)
        return self.linear(out)

    def encode(self, src, src_mask):
        pos_encoded = self.positional_encoding(self.src_emb(src))
        return self.transformer.encoder(pos_encoded, src_mask)

    def decode(self, tgt, memory, tgt_mask):
        pos_encoded = self.positional_encoding(self.tgt_emb(tgt))
        return self.transformer.decoder(pos_encoded, memory, tgt_mask)


device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')






